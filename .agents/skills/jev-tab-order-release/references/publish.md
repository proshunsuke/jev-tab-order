...
The setup below is already in place. Verify or restore it when needed; do not create replacement repositories or store items for normal updates.

1. Verify the destination GitHub repository and its `origin` remote. Commit and push only when requested.
2. Verify the existing Jev Tab Order item and publisher IDs recorded above. Upload updates to that same item; never reuse the reference extension's item ID.
...
| `CHROME_EXTENSION_ID`            | The existing Jev Tab Order item ID                                                                                    |
...
4. Complete dashboard changes and submit for review separately when requested. The existing published version remains available while an update is under review.
...
- If using `npm version`, pass `--no-git-tag-version` to avoid automatic commits and tags. Do not reuse a version already uploaded or published; check the store item and GitHub releases before choosing the next version.
...
2. Verify the existing extension ID and upload the verified ZIP as an update to that item.
